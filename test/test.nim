...
var
  hwys = db_pmis.getAllRows(sql"select * from hwys_trm;")
...
for i, b in lane:
  # echo "line: ", i, " field: ", b
  doAssert(b.startsWith("M") or b.startsWith("F") or b.startsWith("l"))
...
# check routes in PMIS
for i, h in hwy.unique():
  if i > 0:
    # echo h
    doAssert(h in shwy)
echo "+ passed: no unknown routes"
...
for i, e in erm:
  if i > 0:
    #echo fixtrm(e)
    doAssert(fixtrm(e) in strm)
echo "+ passed: good erms"


# To track done trm errors use the following sql: 
...
# TODO errors (NOFIX as seems to be OK)
